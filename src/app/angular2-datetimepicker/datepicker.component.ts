...
@Component({
  ...
  styleUrls: ['./datepicker.component.css', './rangepicker.css'],
...
export class DatePicker implements OnInit, ControlValueAccessor {
  ...
  initDate(val: string) {
    ...
    this.storedDate = _.clone(this.date);
  ...
  initDateRange(val: DateRange) {
    ...
    if (
      this.dateRange.startDate.getHours() == 0 ||
    ...
    if (
      this.dateRange.endDate.getHours() == 0 ||
  ...
  generateYearList(evt: any, param: string) {
    var startYear = null;
  ...
  setTimeView() {
    ...
    this.storedDate = _.cloneDeep(this.date);
  ...
  setDay(evt: any, type: string) {
    if (evt.target.innerHTML) {
      ...
      // replace old selected time
      selectedDay.setHours(this.storedDate.getHours());
      selectedDay.setMinutes(this.storedDate.getMinutes());
      selectedDay.setSeconds(this.storedDate.getSeconds());
  ...
  setMonth(evt: any) {
    if (evt.target.getAttribute('id')) {
      var selectedMonth = this.settings.cal_months_labels_short.indexOf(
  ...
  prevMonth(e: any) {
    ...
    if (this.date.getMonth() == 0) {
      ...
      var prevmonthLength = this.getMonthLength(
  ...
  nextMonth(e: any) {
    ...
    if (this.date.getMonth() == 11) {
      ...
      var nextmonthLength = this.getMonthLength(
        this.date.getMonth() + 1,
        this.date.getFullYear()
      );
      var currentDate = this.date.getDate();
  ...
  closepopover(evt) {
    let openState = false;
    _.forEach(evt.path, (element, i) => {
      if (element.className && element.className.includes('wc-date-popover')) {
        openState = true;
        return false;
      }
    });
